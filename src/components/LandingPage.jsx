...
export default function LandingPage() {
    ...
    useEffect(() => {
        ...
        const subscription = supabase
            ...
            .on(
                'postgres_changes',
                { event: '*', schema: 'public', table: 'doctors', filter: 'role=eq.doctor' },
                () => fetchDoctors()
            )
            .subscribe();
    ...
    return (
        <div className={`min-h-screen ${darkMode ? 'bg-gray-900 text-white' : 'bg-white text-gray-900'
            } transition-colors duration-300 ${language === 'ar' ? 'rtl' : 'ltr'}`}>
            {/* Header */}
            <header className={`fixed top-0 left-0 right-0 ${darkMode ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'
                ...
                <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex justify-between items-center py-4">
                    ...
                    <nav className={`hidden md:flex items-center gap-8 ${darkMode ? 'text-gray-300' : 'text-gray-700'
                        ...
                        <button onClick={() => scrollToSection('home')} className="hover:text-[#0B8FAC] transition">
                            {t.home}
                        </button>
                        <button onClick={() => scrollToSection('about')} className="hover:text-[#0B8FAC] transition">
                            {t.about}
                        </button>
                        <button onClick={() => scrollToSection('doctors')} className="hover:text-[#0B8FAC] transition">
                            {t.doctors}
                        </button>
                        <button onClick={() => scrollToSection('contact')} className="hover:text-[#0B8FAC] transition">
                            {t.contact}
                        </button>
                    </nav>
                ...
                {isMenuOpen && (
                    <div className={`md:hidden py-4 border-t ${darkMode ? 'border-gray-700 bg-gray-800' : 'border-gray-200 bg-white'
                        ...
                        <nav className="flex flex-col gap-4 px-4">
                            <button onClick={() => scrollToSection('home')} className="text-left hover:text-[#0B8FAC]">
                                {t.home}
                            </button>
                            <button onClick={() => scrollToSection('about')} className="text-left hover:text-[#0B8FAC]">
                                {t.about}
                            </button>
                            <button onClick={() => scrollToSection('doctors')} className="text-left hover:text-[#0B8FAC]">
                                {t.doctors}
                            </button>
                            <button onClick={() => scrollToSection('contact')} className="text-left hover:text-[#0B8FAC]">
                                {t.contact}
                            </button>
                            <button
                                onClick={() => handleOpenAuth('doctor')}
            ...
            {/* Hero Section */}
            <section id="home" className={`pt-24 pb-16 ${darkMode ? 'bg-gray-800' : 'bg-linear-to-br from-[#E6F7FB] to-white'
                } transition-colors`}>
                    ...
                    <div>
                        ...
                        <div className="flex flex-wrap gap-4">
                            <button
                                ...
                                className="px-8 py-3 bg-[#0B8FAC] text-white rounded-lg hover:bg-[#0a7a94] transition"
                            >
                                {t.learnMore}
                            </button>
                            <button className={`px-8 py-3 border-2 border-[#0B8FAC] text-[#0B8FAC] rounded-lg transition ${darkMode ? 'hover:bg-gray-700' : 'hover:bg-[#E6F7FB]'
                                }`}>
                                {t.getApp}
                            </button>
                        </div>
                ...
                <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                    ...
                    {loading ? (
                        ...
                        <div className={`text-center py-12 rounded-xl shadow-md ${darkMode ? 'bg-gray-700' : 'bg-white'}`}>
                            <p className={`text-lg ${darkMode ? 'text-gray-300' : 'text-gray-600'}`}>{t.noDoctorsAvailable}</p>
                        </div>
                    ) : (
            ...
            <footer className={`${darkMode ? 'bg-gray-800' : 'bg-[#0B8FAC]'} text-white py-8 text-center transition-colors`}>
                <p>© 2025 {t.memora}. {t.allRightsReserved}.</p>
            </footer>
